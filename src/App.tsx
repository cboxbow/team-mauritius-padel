...
function NewsCard({ item, lead = false }: { item: NewsItem; lead?: boolean }) { return <Link className={`news-card campaign-news-card ${item.featured || lead ? "featured" : ""}${lead ? " newsroom-lead-card" : ""}`} to={`/news/${item.slug}`}><div className="news-image"><img loading="lazy" src={item.image} alt={item.title} /><span className="news-overlay" /><span className="news-cover-label">{item.category}</span></div><div className="news-card-copy"><div className="news-meta"><span>{item.category}</span><span>{item.date}</span></div><h3>{item.title}</h3><p>{item.excerpt}</p><span className="read-more">Read story <ArrowUpRight size={15} /></span></div></Link>; }
